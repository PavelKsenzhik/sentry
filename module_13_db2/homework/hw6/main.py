    ...
    for day, employee in workers_on_day.items():
        for employee_id, sport in employees:
            if WEEK_DAYS[day.weekday()] == WEEK_DAYS[SECTIONS.index(sport)]:
                continue
            if working_days[employee_id] != COUNT_WORKING_DAYS_FOR_EVERYONE_EMPLOYEE + 1:
                cursor.execute(sql_request_insert_employee, (employee_id, str(day)[:10]))
                working_days[employee_id] += 1
